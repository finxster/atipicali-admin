            ...
            <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                  {{ t('serviceTypes.edit.id') }}
                </label>
                <input
                  :value="formData.id"
                  type="text"
                  disabled
                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50 text-gray-500 cursor-not-allowed font-mono text-sm"
                />
              </div>
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                  {{ t('serviceTypes.edit.namePt') }}
                  <span class="text-red-500">*</span>
                </label>
                <input
                  v-model="formData.namePt"
                  type="text"
                  :placeholder="t('serviceTypes.edit.namePtPlaceholder')"
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                  {{ t('serviceTypes.edit.nameEn') }}
                  <span class="text-red-500">*</span>
                </label>
                <input
                  v-model="formData.nameEn"
                  type="text"
                  :placeholder="t('serviceTypes.edit.nameEnPlaceholder')"
                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-atipical-blue focus:border-transparent transition-all"
          ...
          <div class="border-t border-gray-200 px-6 py-4 bg-gray-50">
            <div class="flex items-center justify-end space-x-3">
              ...
              >
                ...
                <span>{{ submitting ? t('serviceTypes.edit.submitting') : t('serviceTypes.edit.submit') }}</span>
              </button>
            </div>
          </div>
        </div>
...
// Watch for service type changes and populate form
watch(() => props.serviceType, (newServiceType) => {
  if (newServiceType) {
    formData.value = {
      id: newServiceType.id,
      namePt: newServiceType.namePt,
      nameEn: newServiceType.nameEn
    }
    errors.value = {}
    generalError.value = ''
  }
...
const validateForm = () => {
  ...
  if (!formData.value.namePt.trim()) {
    errors.value.namePt = t('serviceTypes.edit.errors.namePtRequired')
  }
  
  if (!formData.value.nameEn.trim()) {
    errors.value.nameEn = t('serviceTypes.edit.errors.nameEnRequired')
  }
...
const handleSubmit = async () => {
  ...
  try {
    const response = await apiClient.put(`/api/admin/service-types/${formData.value.id}`, {
    ...
    generalError.value = err.response?.data?.message || t('serviceTypes.edit.errors.general')
  } finally {
